...
import com.vozsegura.security.GatewayRequestValidator;
        ...
        if (cedula == null || userType == null) {
            ...
            if (session != null) {
                ...
                if (authenticated != null && authenticated) {
                    // Buscar primero cedulaHash (nuevo sistema con hash)
                    cedula = (String) session.getAttribute("cedulaHash");
                    if (cedula == null) {
                        // Fallback: buscar cedula (sistema antiguo)
                        cedula = (String) session.getAttribute("cedula");
                    }
                    if (cedula == null) {
                        // Otro fallback: citizenHash para denunciantes
                        cedula = (String) session.getAttribute("citizenHash");
                    }
                    userType = (String) session.getAttribute("userType");

                    log.debug("Session authentication found: userType={}, cedula={}...",
                        userType, cedula != null ? cedula.substring(0, 8) : "NULL");
                }
            }
        }
